...
export default {
  ...
  computed: {
    no_hp() {
      ...
      return fileStore.no_hp || "user@example.com";
    },
    isButtonDisabled() {
  ...
  methods: {
    ...
    async handleSubmit() {
      ...
      try {
        ...
        const response = await api.post("/verify-otp-hp", requestData, {
          headers: { "Content-Type": "application/json" },
        });

        if (response.status === 200 || response.status === 201) {
          console.log("OTP berhasil diverifikasi");
          this.$router.push({ path: "/dashboard/halamanSuksesPembukaanRekeningNTB" });
        } else {
