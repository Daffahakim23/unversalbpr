  ...
  <form @submit.prevent="handleSubmit">
    <div class="flex flex-col items-center">
      ...
      <p class="text-base text-neutral-700 text-center">
        Kami telah mengirimkan kode OTP ke <strong>{{ no_HP }}</strong>. Masukkan kode OTP di bawah untuk melanjutkan.
      </p>
...
export default {
  ...
  computed: {
    no_hp() {
      ...
      return fileStore.no_hp || "Nomor Handphone";
    },
    isButtonDisabled() {
  ...
  methods: {
    ...
    async handleSubmit() {
      ...
      try {
        ...
        const response = await api.post("/verify-otp-hp-pencairan-deposito", requestData, {
          headers: { "Content-Type": "application/json" },
        });

        if (response.status === 200 || response.status === 201) {
          console.log("OTP berhasil diverifikasi");
          this.$router.push({ path: "/dashboard/halamanSuksesPencairanDeposito" });
        } else {
