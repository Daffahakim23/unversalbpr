          ...
          <div v-if="photoUrl" class="mt-4 ">
            <div v-if="documentType === 'npwp'" class="mt-4">
              <FormField label="Nomor NPWP" id="nomornpwp" v-model="nomorNpwp" placeholder="Masukkan Nomor NPWP"
                variant="numeric" required maxlength="16" />
            </div>
  ...
  setup() {
    ...
    const uploadPhoto = async () => {
      ...
      try {
        ...
        const apiEndpoint =
          documentType.value === "ktp" ? "/ocr-ktp-deposito" :
            documentType.value === "tandaTangan" ? "/tt-basah-deposito" :
              documentType.value === "npwp" ? "/npwp-deposito" : "/foto-diri-deposito";
        ...
        if (documentType.value === "ktp") {
          ...
          router.push({ name: "UploadDokumenPenempatanDepositoNTB" });
        }
      } catch (error) {
